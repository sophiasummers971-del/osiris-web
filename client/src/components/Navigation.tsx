  ...
  return (
    <nav className="fixed inset-x-0 top-0 z-50 border-b border-border/60 bg-background/90 backdrop-blur-xl">
      <div className="container flex h-16 items-center justify-between">
        <Link href="/" className="flex items-center gap-3 font-mono font-bold tracking-[0.16em] text-primary">
        ...
        <div className="hidden items-center gap-2 lg:flex">
          <Button variant="ghost" size="icon" asChild aria-label="Open OSIRIS repository">
            <a href="https://github.com/sophiasummers971-del/osiris-web" target="_blank" rel="noopener noreferrer">
          ...
          {isAuthenticated ? (
            ...
            <Button asChild size="sm" variant="outline">
              <a href={getLoginUrl()}><Fingerprint className="mr-2 h-4 w-4" />Authenticate</a>
            </Button>
          )}
        </div>
      ...
      {open && (
        <div className="border-t border-border/60 bg-background px-4 py-4 lg:hidden">
          <div className="container flex flex-col gap-1 p-0">
            {links.map(([label, href]) => (
              <Link key={href} href={href} onClick={() => setOpen(false)} className="px-3 py-3 font-mono text-sm text-muted-foreground hover:bg-card hover:text-primary">
                {label}
              </Link>
            ))}
            {!isAuthenticated && <a href={getLoginUrl()} className="mt-2 px-3 py-3 font-mono text-sm text-primary">Authenticate</a>}
          </div>
        </div>
      )}
    </nav>
